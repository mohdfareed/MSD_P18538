...
Provides an interface for using a file as an audio source. It uses the events
service to broadcast the audio data.
"""
...
def create_file_player(filename: str, chunk_size: int = 1024):
    """Creates a file audio player that returns audio chunks from a file.
...
async def create_websocket_player(websocket: WebSocketConnection):
    """Creates a websocket audio player that returns audio chunks from a
    websocket.
...
def create_mic_player():
    ...
    def receive_audio():
        nonlocal mic
        data = mic.read(
            mic_config.chunk_size * 10, exception_on_overflow=False
        )
        wav_data = io.BytesIO()
